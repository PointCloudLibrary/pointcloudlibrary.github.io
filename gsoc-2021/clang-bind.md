...
- Earlier, we were creating the tree structure using python dictionaries, in which a node had the following structure:
  ...
  ```
  While this worked fine, it could have been improved by using a tree-based python package, instead of handling it via dictionaries. It also helps us reduce code during tree traversal by using pre-defined methods.
...
- In unit tests, we use the [path_to_leaves](https://treelib.readthedocs.io/en/latest/treelib.html#treelib.tree.Tree.paths_to_leaves) function to imitate the prior tests' structure. We also now use clang objects directly for assertions (earlier, we were using string comparisons).
...
- Earlier, since this project was meant to have resided in the [pcl](https://github.com/PointCloudLibrary/pcl) repo, `clang-bind` was designed for integration within that.
- Now, that we aim to create a general-purpose tool which is not specific to `pcl`, we updated the project to have the desired directory structure, for easier imports and better segregation:
  ...
  └── third_party
      └── conan.cmake
  ```
- We perform clean-up: removing unnecessary old files, using `PYTHONPATH` instead of `context.py`, etc.
